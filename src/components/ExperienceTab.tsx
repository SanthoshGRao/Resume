...
export default function ExperienceTab() {
  ...
  return (
    ...
    >
      ...
      <motion.div variants={itemVariants} className="border border-slate-800 bg-slate-900/10 rounded-xl p-6 flex flex-col gap-5">
        {/* Company Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-900">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-lg border border-slate-800 bg-slate-950 text-blue-400">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1 rounded bg-slate-950 border border-slate-800 text-[10px] font-mono text-slate-400 sm:self-start">
        ...
        <div className="flex flex-wrap items-center gap-2 text-[10px] text-slate-500">
          ...
          {experienceData.technologies.map((tech, i) => (
            <span key={i} className="bg-slate-950 border border-slate-900 text-slate-400 font-mono text-[9px] px-2 py-0.5 rounded">
              {tech}
            </span>
          ))}
        </div>
